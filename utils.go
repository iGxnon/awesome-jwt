...
type Keys struct {
	Keys []struct {
		...
		Alg string `json:"alg,omitempty"` // algorithm  ep. RS256
...
// 支持 EC/RSA
func ParsePathToKey(path string, kid string) (interface{}, error) {
	resp, err := http.Get(path)
	if err != nil {
		return nil, errors.New("cannot get resp, maybe it does not exist")
	}
	defer resp.Body.Close()
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return nil, errors.New("error while reading resp.Body")
	}
	var keys = &Keys{}
	...
	return nil, errors.New("cannot find this key")
}
